...
[Durable Objects](https://www.cloudflare.com/developer-platform/products/durable-objects/) are one of the most powerful primitives in the Cloudflare Developer Platform. I have seen many people struggle to define Durable Objects. I see them as **mini servers without the ops**.
...
1. **Scalability** - Sharding a database in one instance per user addresses the limitations of storing and managing large datasets on a single database server or instance
...
4. **Performance** - Durable Objects are distributed globally, and instanciated closest to the first request they receive. As such, each database will be instanciated closes to its uers, significantly reducing end-to-end latency
...
Now let's see how our "one database per user" pattern works in practice. After you have deployed your Worker using `wrangler`, you'll get a URL for your Worker: `https://durable-objects-database-per-user.<account-name>.workers.dev`, where `<account-name>` is the name of your account.
